...
def select_mujoco_render_backend(env_num_per_gpu: int) -> str:
    """Use EGL for one worker per GPU and OSMesa for concurrent workers."""
    if env_num_per_gpu <= 0:
        raise ValueError("env_num_per_gpu must be positive.")
    return "egl" if env_num_per_gpu == 1 else "osmesa"
...
def configure_mujoco_worker_env(
    ...
    backend = select_mujoco_render_backend(env_num_per_gpu)
