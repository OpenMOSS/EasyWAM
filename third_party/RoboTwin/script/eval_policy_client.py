...
def _result_suffix_from_task_config(task_config):
    ...
    if task_config == "demo_randomized":
        return "random"
    raise ValueError(f"Unsupported task_config for fixed result naming: {task_config}")
...
def main(usr_args):
    ...
    if eval_num_episodes <= 0:
        raise ValueError(f"eval_num_episodes must be positive, got {eval_num_episodes}")
    ...
    model = ModelClient(
        port=port,
        timeout=int(usr_args.get("model_server_timeout", 600)),
    )
    st_seed, suc_num, timing_summary = eval_policy(task_name,
                                   TASK_ENV,
                                   args,
                                   model,
                                   st_seed,
    ...
    if timing_enabled:
        timing_path = save_dir / f"_timing_{result_suffix}.json"
        timing_path.write_text(json.dumps(timing_summary, indent=2), encoding="utf-8")
        print(f"Timing data has been saved to {timing_path}")
...
def eval_policy(task_name,
    ...
    now_seed = st_seed
    task_total_reward = 0
    timing_totals = {"prompt_encode_s": 0.0, "infer_s": 0.0, "sim_s": 0.0}
    ...
    while succ_seed < test_num:
        ...
        succ = False
        model.call(func_name="reset")
        while TASK_ENV.take_action_cnt < TASK_ENV.step_lim:
            need_obs = True
            if skip_get_obs_within_replan:
                need_obs = bool(model.call(func_name="should_request_observation"))
            observation = TASK_ENV.get_obs() if need_obs else None
            action = model.call(
                func_name="remote_step",
                obs={
                    "observation": observation,
                    "instruction": TASK_ENV.get_instruction(),
                },
            )
            sim_started = time.perf_counter() if timing_enabled else 0.0
            TASK_ENV.take_action(np.asarray(action), action_type="qpos")
            if timing_enabled:
                timing_totals["sim_s"] += time.perf_counter() - sim_started
            if TASK_ENV.eval_success:
                succ = True
                break
        if timing_enabled:
            rollout_timing = model.call(func_name="get_timing_rollout")
            timing_totals["prompt_encode_s"] += float(
                rollout_timing.get("prompt_encode_s", 0.0)
            )
            timing_totals["infer_s"] += float(rollout_timing.get("infer_s", 0.0))
    ...
    return now_seed, TASK_ENV.suc, timing_summary
